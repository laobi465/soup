<template>
  <div class="admin-order">
    ...
    <el-table :data="tableData" v-loading="loading" border stripe>
      ...
      <el-table-column prop="merchant_name" label="商户" min-width="120" />
    ...
    <el-dialog v-model="detailVisible" title="订单详情" width="600px">
      <el-descriptions v-if="currentOrder" :column="2" border>
        ...
        <el-descriptions-item label="商户">{{ currentOrder.merchant_name }}</el-descriptions-item>
        ...
        <el-descriptions-item label="第三方流水号">{{ currentOrder.pay_trade_no }}</el-descriptions-item>
        <el-descriptions-item label="佣金金额">¥{{ currentOrder.commission_amount }}</el-descriptions-item>
...
import { getAdminOrderList, getAdminOrderDetail, adminOrderRefund } from '@/api/admin/payment'
...
function handleRefund(row) {
  ElMessageBox.confirm(`确定要退款订单「${row.order_no}」吗？`, '提示', {
    type: 'warning',
    confirmButtonText: '确定',
    cancelButtonText: '取消'
  }).then(async () => {
    const res = await adminOrderRefund({ order_id: row.id, reason: '后台操作退款' })
    if (res.code === 0) {
